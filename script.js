...
function adicionarItemInventario(categoria) {
    const nome = document.getElementById(`${categoria === 'materiais' ? 'mat' : 'ferr'}-nome-input`).value.trim();
    if (!nome) return alert("Digite o nome.");
    const sub = subTabAtiva[categoria];
    DB_INVENTARIO[categoria][sub].push({ id: Date.now(), nome: nome, quantidade: 0, unidade: 'Und', data: new Date().toLocaleDateString('pt-BR') });
    salvarBD();
    renderizarInventario(categoria);
}
...
function renderizarInventario(categoria) {
    ...
    if (!tbody) return;
    tbody.innerHTML = DB_INVENTARIO[categoria][sub].map(item => `
        <tr data-id="${item.id}" data-cat="${categoria}" data-sub="${sub}">
            ...
            <td><button onclick="removerItemInventario('${categoria}', '${sub}', ${item.id})">X</button></td>
        </tr>
    `).join('');
}
...
function lidarBotaoLogin() {
    ...
    if (!isAdminLogado) {
        ...
        if (senha === "admin") { isAdminLogado = true; sidebar.style.display = 'flex'; }
    } else {
        isAdminLogado = false; sidebar.style.display = 'none';
    }
}
...
function salvarNomeNesteCelular() {
    const nome = document.getElementById('input-nome-celular').value;
    localStorage.setItem('cc_nome_proprio_dispositivo', nome);
    location.reload();
}
...
function carregarProjetoPlanta(event) {
    ...
    if (!arquivo) return;
    const img = document.getElementById('viewer-img-projeto');
